...
def coverage_status(data: dict) -> tuple[int, int, bool]:
    providers = [row for row in data.get("providers", []) if isinstance(row, dict)]
    counties = [row for row in data.get("counties", []) if isinstance(row, dict)]
    valid_fips = {
        str(row.get("f", ""))
        for row in counties
        if re.fullmatch(r"\d{5}", str(row.get("f", "")))
    }
    covered = {
        str(fips)
...
def update_sitemap(targets: set[str], *, include: bool) -> None:
    ...
    for node in list(root):
        ...
        if loc is not None:
            existing.setdefault((loc.text or "").strip(), []).append(node)
    for target in targets:
        if include:
            ...
            for node in existing.get(target, []):
                root.remove(node)
...
def repair_public_links() -> None:
    ...
    home_text = home_text.replace(
        ...
        "/guides/find-septic-system-records/",
    )
    ...
    required = (
        ...
        "/assets/county-lookup.js",
        "fips 48121",
    )
    missing = [marker for marker in required if marker not in county_lower]
    if missing:
        raise RuntimeError(
            "County directory is missing ZIP/city/county/FIPS lookup markers: "
            + ", ".join(missing)
        )
...
def hide_incomplete_directories(provider_count: int, covered_count: int) -> None:
    ...
    update_sitemap(
        {f"{DOMAIN}/septic-services-near-me/", f"{DOMAIN}/providers/"},
        include=False,
    )
    repair_public_links()
...
def verify_hidden() -> None:
    ...
    county_lower = counties.lower()
    if "data-county-lookup-root" not in county_lower or "fips 48121" not in county_lower:
        raise RuntimeError("County directory is missing the national county/FIPS lookup")
...
def main() -> None:
    ...
    if complete:
        ...
        print(
            ...
            "county/FIPS lookup remains the primary public location search"
        )
